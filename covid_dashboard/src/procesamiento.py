...
from lectura import Ui_MainWindow
...
class MatplotlibWidget(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Dashboard Covid 2022 by Karen Masache Astudillo ")  # Título de MainWindows
        self.addToolBar(NavigationToolbar(self.ui.visualizacion.canvas, self))  # agregando Axes
        ...
        self.plotAmbosPaises()  # graficando de inicio Muertes y casos acumulativos
    ...
    def getCSV(self):
        #leyendo csv
        data_path = 'data/covid_data_official.csv'
        self.data_fr = pd.read_csv(data_path)
        self.dataReady = self.data_fr
        ...
        # Cargar estados
        self.loadEstados()
        # Método para cargar los estados en combo box y agregando la palabra All al inicio de la lista
    def loadEstados(self):
        # Borrar Axes
        self.ui.visualizacion.canvas.axes.cla()
        self.ui.visualizacion.canvas.axes1.cla()
        listEstado = []
    ...
    def DatosDiarios(self):
       ...
       # Esta función permite escoger los datos acumulados en relación a la sentencia de control que cumpla las condiciones y se compara radio botton
    ...
    def btnstate(self):
       ...
       #Detecta la selección de un nuevo estado para compararlo con los radio buttons que esten  seleccionados para realizar la gráfica correspondiente
    ...
    #Verificando el radio button selccionado y en relación a eso invocar a los métodos que cumplan con la  condición correcta
    ...
    # invocando a otras funciones que realizan la gráfica de casos covid por paises y estados, según las condiciones que se cumplan de los combo Box seleccionados y los radio buttons.
    ...
    #invocar a otras funciones que realizan la gráfica de muertes  covid por  paises y estados, según las condiciones que se cumplan de los combo Box seleccionados y los radio buttons.
    ...
    def selectPlotMuertes(self):
        ...
        #invocar a otras funciones que realizan la gráfica de los casos y muertes covid por paises y estados, según las condiciones que se cumplan de los combo Box seleccionados y los radio buttons.
    ...
    def plotPaisCasos(self):
        ...
        for i in range(len(estadoCasos)):
            if (estadoCasos[i] == 'All'):
                casosList.append(y[i]) # Se agregan los casos extraidos
                fechaListCasos.append(x[i]) # Se agrega la columna de fechas
                datosMuertes.append(' ') # se agregan valores vacios para que los datos se ubiquen en cada columna

        slider = self.rollingMean() # Se obtiene el valor del slider
        self.ui.lcdNumberSlider.display(slider) # el valor recuperado del slider pasarlo al  leddisplay

        # Convirtiendo dataFrames para utilizar la función rolling para la media móvil
        df_CasosAcumulativos = pd.DataFrame(casosList, columns=['Casos'])
        rolling_mean = df_CasosAcumulativos.rolling(window=slider).mean()

        # Si esto es verdadero se presentará la gráfica de casos covid con datos acumulativos
        if self.ui.radioButtonCasos.isChecked() and self.ui.radioButton_2.isChecked():
            ...
            self.ui.visualizacion.canvas.axes.plot(fechaListCasos, casosList, 'r', label="Casos") # Graficas casos covid
            self.ui.visualizacion.canvas.axes.legend() # agregando etiquetas
            self.ui.visualizacion.canvas.axes.set_title("Casos Acumulados Covid pais de " + pais) # Titulo de la gráfica
            self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
            self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de Casos ")  # insertando el título del eje Y

            # si slider cambia valor presentará una nueva grafica en relación a la MM.
            if self.ui.sliderPromedio.value() > 1:
                self.ui.visualizacion.canvas.axes.cla()
                self.ui.visualizacion.canvas.axes.plot(fechaListCasos, rolling_mean, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes.legend() # Agregando etiquetas
                self.ui.visualizacion.canvas.axes.set_title("Casos Acumuladas Promedio Covid pais de " + pais)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
        ...
        # Si se cumple la condición se presenta la gráfica con datos diarios
            ...
            self.ui.visualizacion.canvas.axes.plot(fechaListCasos, casosArray, 'r', label="Casos")
            self.ui.visualizacion.canvas.axes.legend() # agregando etiquetas
            self.ui.visualizacion.canvas.axes.set_title("Casos Diarios Covid pais de " + pais)
            self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
            self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de Casos ")  # insertando el título del eje Y

            # convirtiendo de array a dataFrame
            df_MuertesDiarias = pd.DataFrame(casosArray, columns=['Casos'])
            rolling_mean2 = df_MuertesDiarias.rolling(window=slider).mean()

            # graficando la media móvil con datos diarios
            if self.ui.sliderPromedio.value() > 1:
                self.ui.visualizacion.canvas.axes.cla()
                self.ui.visualizacion.canvas.axes.plot(fechaListCasos, rolling_mean2, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes.legend() # agregando etiquetas
                self.ui.visualizacion.canvas.axes.set_title("Casos Diarias Promedio Covid pais de " + pais)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y

        self.ui.visualizacion.canvas.draw() # dibujando la gráfica

    
    #graficando los datos de los estados selccionados
    def plotEstadosCasos(self):
        self.clearRows() # Borrar datos de la tabla, para llenar con los nuevos datos

        # Borrando las gráficas del Widget
        self.ui.visualizacion.canvas.axes.cla()
        self.ui.visualizacion.canvas.axes1.cla()
        ...
        if estado == 'All':
         ...
         # Caso contrario realiza un filtro de datos de acuerdo al estado selccionado, obteniendo Fecha y casos de estados
            ...
            # Presentando grafica de estados de forma acumulativa
            if self.ui.radioButtonCasos.isChecked() and self.ui.radioButton_2.isChecked():
                self.tableWidget(datosCasos, datosMuertes, fecha)  # Datos tabla
                self.ui.visualizacion.canvas.axes.plot(x, y, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes.legend() # Agrega etiquetas
                self.ui.visualizacion.canvas.axes.set_title("Casos Acumulados Covid " + estado)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de Casos ")  # insertando el título del eje Y

                # Realizando  grafica de acuerdo al numero de dias seleccionado en el slider de forma acumulativa
                if self.ui.sliderPromedio.value() > 1:
                    self.ui.visualizacion.canvas.axes.cla()
                    self.ui.visualizacion.canvas.axes1.cla()
                    self.ui.visualizacion.canvas.axes.plot(x, rolling_mean, 'r', label="Casos")
                    self.ui.visualizacion.canvas.axes.legend()
                    self.ui.visualizacion.canvas.axes.set_title("Casos Promedio Acumulados Covid " + estado)
                    self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                    self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de Casos ")  # insertando el título del eje Y

            # Presentando  gráficas de muertes de forma diaria
            elif self.ui.radioButtonCasos.isChecked() and self.ui.radioButtonDiarios.isChecked():
                ...
                self.ui.visualizacion.canvas.axes.plot(x, casosArray, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes.legend()
                self.ui.visualizacion.canvas.axes.set_title("Casos Diarios Covid pais de " + estado)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de Casos ")  # insertando el título del eje Y

                # convirtiendo de lista a Dataframe para obntener la media móvil con la función rolling
                df_MuertesDiarias = pd.DataFrame(casosArray, columns=['Casos'])
                rolling_mean2 = df_MuertesDiarias.rolling(window=slider).mean()

                # graficando media móvil con datos diarios
                if self.ui.sliderPromedio.value() > 1:
                    self.ui.visualizacion.canvas.axes.cla()
                    self.ui.visualizacion.canvas.axes1.cla()
                    self.ui.visualizacion.canvas.axes.plot(x, rolling_mean2, 'r', label="Casos")
                    self.ui.visualizacion.canvas.axes.legend()
                    self.ui.visualizacion.canvas.axes.set_title("Casos Promedio Diarios Covid de " + estado)
                    self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                    self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de Casos ")  # insertando el título del eje Y
            self.ui.visualizacion.canvas.draw()

    
    #Esta función permite graficar los datos de los estados seleccionados respecto a muertes
    ...
    def plotEstadoMuertes(self):
        ...
        if estado == 'All':
            ...
            if self.ui.radioButtonMuertes.isChecked() and self.ui.radioButton_2.isChecked():
                ...
                self.ui.visualizacion.canvas.axes.plot(x, y, label="Muertes")
                self.ui.visualizacion.canvas.axes.legend()
                self.ui.visualizacion.canvas.axes.set_title("Muertes Acumuladas Covid de " + estado)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
                if self.ui.sliderPromedio.value() > 1:
                    self.ui.visualizacion.canvas.axes.cla()
                    self.ui.visualizacion.canvas.axes.plot(x, rolling_mean, label="Muertes")
                    self.ui.visualizacion.canvas.axes.legend()
                    self.ui.visualizacion.canvas.axes.set_title("Muertes Promedio Acumuladas Covid de " + estado)
                    self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                    self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
            elif self.ui.radioButtonMuertes.isChecked() and self.ui.radioButtonDiarios.isChecked():
                ...
                for i in range(len(y)):
                    datosCasos.append(' ')
                self.tableWidget(datosCasos, datosMuertes, fecha)  # Datos tabla
                self.ui.visualizacion.canvas.axes.plot(x, muertesArray, label="Muertes")

                self.ui.visualizacion.canvas.axes.legend()
                self.ui.visualizacion.canvas.axes.set_title("Muertes Diarias Covid de " + estado)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
                ...
                if self.ui.sliderPromedio.value() > 1:
                    self.ui.visualizacion.canvas.axes.cla()
                    self.ui.visualizacion.canvas.axes.plot(x, rolling_mean2, label="Muertes")
                    self.ui.visualizacion.canvas.axes.legend()
                    self.ui.visualizacion.canvas.axes.set_title("Muertes Promedio Diarias Covid de " + estado)
                    self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                    self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
          ...
          #  Esta función permite realizar la gráfica de muertes covid según el pais selccionado
    ...
    def plotPaisMuertes(self):
        ...
        if self.ui.radioButtonMuertes.isChecked() and self.ui.radioButton_2.isChecked():
            ...
            self.ui.visualizacion.canvas.axes.plot(fechaListMuertes, muertesList, label="Muertes")
            self.ui.visualizacion.canvas.axes.legend()
            self.ui.visualizacion.canvas.axes.set_title("Muertes Acumuladas Covid pais de " + pais)
            self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
            self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y

            if self.ui.sliderPromedio.value() > 1:
                self.ui.visualizacion.canvas.axes.cla()
                self.ui.visualizacion.canvas.axes.plot(fechaListMuertes, rolling_mean, label="Muertes")
                self.ui.visualizacion.canvas.axes.legend()
                self.ui.visualizacion.canvas.axes.set_title("Muertes Acumuladas Promedio Covid pais de " + pais)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
            ...
            self.ui.visualizacion.canvas.axes.plot(fechaListMuertes, muertesArray, label="Muertes")

            self.ui.visualizacion.canvas.axes.legend()
            self.ui.visualizacion.canvas.axes.set_title("Muertes Diarias Covid pais de " + pais)
            self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
            self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
            df_MuertesDiarias = pd.DataFrame(muertesArray, columns=['Muertes'])
            rolling_mean2 = df_MuertesDiarias.rolling(window=slider).mean()
            if self.ui.sliderPromedio.value() > 1:
                self.ui.visualizacion.canvas.axes.cla()
                self.ui.visualizacion.canvas.axes.plot(fechaListMuertes, rolling_mean2, label="Muertes")
                self.ui.visualizacion.canvas.axes.legend()
                self.ui.visualizacion.canvas.axes.set_title("Muertes Diarias Promedio Covid pais de " + pais)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
        self.ui.visualizacion.canvas.draw()


      #   Esta función permite realizar gráficas de muertes y casos por pais
    ...
    def plotAmbosPaises(self):
        ...
        if self.ui.radioButtonAmbos.isChecked() and self.ui.radioButton_2.isChecked():
            ...
            print("Entro acumulados paises")
            self.ui.visualizacion.canvas.axes.plot(fechaListCasos, casosList, 'r-', label="Casos")
            self.ui.visualizacion.canvas.axes1.bar(fechaListMuertes, muertesList)
            self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Acumulados Covid pais de " + pais)
            self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
            self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
            self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
            if self.ui.sliderPromedio.value() > 1:
                self.ui.visualizacion.canvas.axes.cla()
                self.ui.visualizacion.canvas.axes1.cla()
                self.ui.visualizacion.canvas.axes.plot(fechaListCasos, rolling_mean_CA, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes1.plot(fechaListMuertes, rolling_mean_MA, label="Muertes")
                self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Promedio Acumulados Covid pais de " + pais)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
                self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
        elif self.ui.radioButtonAmbos.isChecked() and self.ui.radioButtonDiarios.isChecked():
            ...
            self.ui.visualizacion.canvas.axes.plot(fechaListCasos, casosArray, 'r', label="Casos")
            self.ui.visualizacion.canvas.axes1.plot(fechaListMuertes, muertesArray, label="Muertes")
            self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Diarios pais de " + pais)
            self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
            self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
            self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
            ...
            if self.ui.sliderPromedio.value() > 1:
                self.ui.visualizacion.canvas.axes.cla()
                self.ui.visualizacion.canvas.axes1.cla()
                self.ui.visualizacion.canvas.axes.plot(fechaListCasos, rolling_mean_CD, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes1.plot(fechaListMuertes, rolling_mean_MD, label="Muertes")
                self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Promedio Diarios pais de " + pais)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
                self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
    ...
    def plotAmbosEstados(self):
        ...
        if estado == 'All':
            ...
            if self.ui.radioButtonAmbos.isChecked() and self.ui.radioButton_2.isChecked():
                ...
                self.ui.visualizacion.canvas.axes.plot(x, y, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes1.plot(x1, y1, label="Muertes")

                self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Covid de " + estado)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
                self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
                if self.ui.sliderPromedio.value() > 1:
                    self.ui.visualizacion.canvas.axes.cla()
                    self.ui.visualizacion.canvas.axes1.cla()
                    self.ui.visualizacion.canvas.axes.plot(x, rolling_mean_CA, 'r', label="Casos")
                    self.ui.visualizacion.canvas.axes1.plot(x1, rolling_mean_MA, label="Muertes")
                    self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Promedio Covid de " + estado)
                    self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                    self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
                    self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
            elif self.ui.radioButtonAmbos.isChecked() and self.ui.radioButtonDiarios.isChecked():
                ...
                self.tableWidget(datosCasos, datosMuertes, fecha)  # Datos tabla
                self.ui.visualizacion.canvas.axes.plot(x, casosArray, 'r', label="Casos")
                self.ui.visualizacion.canvas.axes1.plot(x1, muertesArray, label="Muertes")

                self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Diarios  de " + estado)
                self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
                self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
                df_MuertesDiarias = pd.DataFrame(casosArray, columns=['Muertes'])
                ...
                if self.ui.sliderPromedio.value() > 1:
                    self.ui.visualizacion.canvas.axes.cla()
                    self.ui.visualizacion.canvas.axes1.cla()
                    self.ui.visualizacion.canvas.axes.plot(x, rolling_mean_CD, 'r', label="Casos")
                    self.ui.visualizacion.canvas.axes1.plot(x1, rolling_mean_MD, label="Muertes")
                    self.ui.visualizacion.canvas.axes.set_title("Muertes y Casos Promedio Diarios  de " + estado)
                    self.ui.visualizacion.canvas.axes.set_xlabel("Fecha")  # insertando el título del eje X
                    self.ui.visualizacion.canvas.axes.set_ylabel("Cantidad de casos")  # insertando el título del eje Y
                    self.ui.visualizacion.canvas.axes1.set_ylabel("Cantidad de muertes")  # insertando el título del eje Y
